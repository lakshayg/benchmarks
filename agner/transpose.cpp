...
static void BM_TransposeSimple(benchmark::State& state) {
  ...
  state.SetBytesProcessed((int64_t)state.iterations() * size * size * sizeof(double));
}
 ...
 *    Set 1      Set 2      Set 3
 *
 * 0-64 bytes map to set 1, 64-128 map to set 2 and so on. Once we reach the last
 * set, we cycle back to 1. Within a set, cache line is selected based on a policy
 * like Least Recently Used (LRU), Not Most Recently Used (NMRU) etc.
 *
 * If we access memory addresses separated by (cache line size * number of sets)
 * bytes, then both the memory locations will be mapped to the same cache set. In
 * the transpose example, when the matrix size is a multiple of this critical size,
 * we end up hitting only a small number of cache sets rather than utilizing the
 * entire cache thus causing contention for cache and reducing performance severely
 */
